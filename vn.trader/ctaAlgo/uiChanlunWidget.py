    ...
    def findTopAndLow(self, after_fenxing):
        ...
        while (i < len(after_fenxing) - 1):
            case1 = after_fenxing.high[i - 1] < after_fenxing.high[i] and after_fenxing.high[i] > after_fenxing.high[i + 1] \
                    and after_fenxing.low[i - 1] < after_fenxing.low[i] and after_fenxing.low[i] > after_fenxing.low[i + 1]  # 顶分型
            case2 = after_fenxing.low[i - 1] > after_fenxing.low[i] and after_fenxing.low[i] < after_fenxing.low[i + 1] \
                    and after_fenxing.high[i - 1] > after_fenxing.high[i] and after_fenxing.high[i] < after_fenxing.high[i + 1]  # 底分型
            if case1:
                ...
                if temp_type == 2:  # 如果上一个分型为底分型，则进行比较，选取低点更低的分型
                    ...
                    if temp_high <= after_fenxing.low[i]:  # 如果上一个顶分型的底比当前底分型的底低，则跳过当前底分型。
                        ...
                        fenxing_data = pd.concat(
                            [fenxing_data, after_fenxing[temp_num:temp_num + 1]], axis=0)
