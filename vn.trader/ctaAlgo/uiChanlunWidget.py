...
class KLineWidget(QtGui.QWidget):
    ...
    def updateMarketData(self, event):
        ...
        print 'time:',tick.time,'symbol:',tick.symbol
...
class TickWidget(QtGui.QWidget):
    ...
    def registerEvent(self):
        """注册事件监听"""
        self.__chanlunEngine.putChanlunEvent
        self.signal.connect(self.updateMarketData)
        self.__eventEngine.register(EVENT_MARKETDATA, self.signal.emit)
...
class ChanlunEngineManager(QtGui.QWidget):
    ...
    def __init__(self, chanlunEngine, eventEngine, mainEngine, parent=None):
        ...
        # 记录日志
        self.__chanlunEngine.writeChanlunLog(u'缠论引擎启动成功')
    ...
    def initUi(self):
        ...
        # k线图
        self.KLineW = KLineWidget(self.__eventEngine, self.__chanlunEngine, self)
        ...
        tickButton = QtGui.QPushButton('Tick')
        ...
        graph.addTab(self.KLineW,u'K线')

        vbox1 = QtGui.QVBoxLayout()

        vbox2 = QtGui.QVBoxLayout()
        vbox1.addWidget(graph)
        vbox2.addWidget(tickButton)
    ...
    def updateSymbol(self):
        """合约变化"""
        # 读取组件数据
        symbol = str(self.codeEdit.text())
        # 获取合约
        # instrument = self.__chanlunEngine.selectInstrument(instrumentid)

        # 重新注册事件监听
        self.__eventEngine.unregister(EVENT_TICK + self.symbol, self.signal.emit)
        self.__eventEngine.register(EVENT_TICK + symbol, self.signal.emit)
        ...
        if contract:
            ...
            self.__mainEngine.subscribe(req, contract.gatewayName)
        else:
            self.writeCtaLog(u'交易合约%s无法找到' % (symbol))
    ...
    def segment(self):
        """加载分段"""
        self.__chanlunEngine.writeChanlunLog(u'分段')
    ...
    def shop(self):
        """加载买卖点"""
        self.__chanlunEngine.writeChanlunLog(u'买卖点')
    ...
    def restore(self):
        ...
        self.__chanlunEngine.writeChanlunLog(u'还原')

    # ----------------------------------------------------------------------
    def pen(self):
        """加载分笔"""
        if not self.pen:
            self.__chanlunEngine.loadSetting()
     #       self.initStrategyManager()  此处应该修改成分笔的函数
        self.pen = True
        self.__chanlunEngine.writeChanlunLog(u'分笔加载成功')

    # ----------------------------------------------------------------------
    def openTick(self):
        """打开Tick图"""
        if not self.tickLoaded:
            print ''
        self.tickLoaded = True
        self.__chanlunEngine.writeChanlunLog(u'打开Tick图')

    # ----------------------------------------------------------------------
    def updateChanlunLog(self, event):
        """更新缠论相关日志"""
        print "Log update"
        #self.__eventEngine.register(EVENT_CHANLUN_LOG, self.signal.emit)
